...
class AstarQ{
	public:
		...
		AstarQ(const AstarQ& elem):current_node(elem.current_node),fcost(elem.current_node),parent_node(elem.parent_node),gcost(elem.gcost){}

		friend ostream& operator<<(ostream& o, AstarQ elem){
...
int main(){
	...
	while(!apq.empty() && apq.top().current_node != n - 1){
		...
		if(unlocked[current_node] == 1){
			if(apq.top().current_node != SOURCE){
				if(path.back() != apq.top().parent_node){
					while(!path.empty() && path.back() != apq.top().parent_node){
						path.pop_back();
					}
				}				
			}
